function w = parzenwin_2(n, widthParam)
% param can be between 0 and 2
...
if widthParam<0
    fprintf('widthParam<0, no filtering\n');
    w = ones(n,1);
else   
    % Index vectors
    k = -(n-1)/2:(n-1)/2;
    ...
    w1 = 2 * (1-abs(k1)/(n/widthParam)).^3;
    w2 = 1 - 6*(abs(k2)/(n/widthParam)).^2 + 6*(abs(k2)/(n/widthParam)).^3; 
    w = [w1 w2 w1(end:-1:1)]';
end
