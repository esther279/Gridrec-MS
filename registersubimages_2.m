...
%     end
    ...
    if min(x2+delta(2))<1
        display('Refining range in lower x')
    ...
    if max(x2+delta(2))>nr2
        display('Refining range in upper x')
    ...
    if min(y2+delta(1))<1
        display('Refining range in lower y')
    ...
    if max(y2+delta(1))>nc2
        display('Refining range in upper y')
...
if upsamp > 1,
    ...
    regionsout.y2 = y2;
    %counter;
    display(['Final registration values, delta = (' num2str(delta(1)) ',' num2str(delta(2)) ')'])
end
